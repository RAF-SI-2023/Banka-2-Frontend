...
export class MarginAccountsService {
  ...
  getMarginsAccount(id:number){
    return this.httpClient.get<MarginAccountDto>(
			`${environment.bankServiceApiUrl}${ApiRoutes.marginsAccountService.getMarginsAccount}`+'/'+id,
		);
  }  

  getAllById(id:number){
    return this.httpClient.get<MarginAccountDto[]>(
			`${environment.bankServiceApiUrl}${ApiRoutes.marginsAccountService.getAllByUserId}`+'/'+id,
		);
  } 

  getAllByEmail(email:string){
    return this.httpClient.get<MarginAccountDto[]>(
			`${environment.bankServiceApiUrl}${ApiRoutes.marginsAccountService.getAllByEmail}`+'/'+email,
		);
  } 

  getAllbyAccountNumber(accountNumber:number){
    return this.httpClient.get<MarginAccountDto[]>(
			`${environment.bankServiceApiUrl}${ApiRoutes.marginsAccountService.getAllByAccountNumber}`+'/'+accountNumber,
		);
  } 
  //PUT
  putMarginsAccount(id:number, marginAccountDto:MarginAccountDto){
    return this.httpClient.post<MarginAccountDto>(
      `${environment.bankServiceApiUrl}${ApiRoutes.marginsAccountService.putMarginsAccount}`+'/'+id,
      marginAccountDto,
    );
  }
  //DELETE
  deleteMarginsAccount(id:number){
    return this.httpClient.delete<MarginAccountDto>(
      `${environment.bankServiceApiUrl}${ApiRoutes.marginsAccountService.deleteMarginsAccount}`+'/'+id,
    );
  }
  //PATCH
