...
export class AcquiredSecuritiesComponent implements OnInit, AfterViewInit {
  ...
  viewSecurity(row: SecurityDto): void {
    const dialogRef  = this.dialog.open(AcquiredSecuritiesInfoDialogComponent, {
      data: row,
      autoFocus: false,
    });
      ...
      .pipe(
        map(response => {
          ...
          if (this.accounts.length > 0) {
            const firstAccountNumber = this.accounts[0].accountNumber;
            this.senderAccountNumber.setValue(firstAccountNumber);
            this.getAllSecuritiesByAccountNumber(firstAccountNumber);
          }
          return response;
        }),
        catchError(error => {
